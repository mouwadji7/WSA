...
function SoumissionFaites({ formData, formDataV, onReturnToHome }) {
  return (
    <main className="container mt-5 pt-5 text-center">
      ...
      <p>
        Votre demande de déménagement a été soumise avec succès. Un numéro de
        soumission a été généré :<strong>{formData.submissionReference}</strong>
        .
      </p>
      ...
      <div className="mt-4">
        <p>
          Vous pouvez vérifier l'état de votre soumission en cliquant sur "État
          de ma soumission" dans le menu de navigation. Nous vous contacterons
          dans les plus brefs délais pour vous fournir un devis (prix) final.
          Merci d'avoir fait une soumission avec Mouwadji Inc.
        </p>
        <button className="btn btn-primary mt-3" onClick={onReturnToHome}>
          Revenir à l'accueil
        </button>
      </div>
    </main>
